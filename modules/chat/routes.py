...
from modules.chat import service_chat
from modules.chat.util import get_current_user_id

# Route handlers delegate chat data access to service_chat.
...
def index():
    ...
    threads = service_chat.get_threads_for_user(me_id)
    thread_ids = [t.thread_id for t in threads]
    members = service_chat.get_members_for_threads(thread_ids)
    ...
    users_by_id = service_chat.get_users_by_ids(list({m.user_id for m in members} | {b.created_by for b in broadcasts}))

    members_by_thread: dict[int, list[ChatThreadMember]] = {}
    for m in members:
        members_by_thread.setdefault(m.thread_id, []).append(m)

    message_counts = service_chat.get_message_counts(thread_ids)
    unread_counts = service_chat.get_unread_counts(thread_ids, me_id)
    broadcast_counts = service_chat.get_message_counts(broadcast_ids)
...
def new_chat():
    ...
    if users:
        ...
        for u in users:
            ...
            u.display_label = service_chat.user_label(u, prof)
...
def thread(thread_id: int):
    ...
    is_member = service_chat.is_member(thread_id, me_id)
    ...
    threads = service_chat.get_threads_for_user(me_id)
    thread_ids = [t.thread_id for t in threads]
    members = service_chat.get_members_for_threads(thread_ids + [thread_id])
    ...
    users_by_id = service_chat.get_users_by_ids(list({m.user_id for m in members} | {t.created_by} | {b.created_by for b in broadcasts}))
    ...
    msgs = service_chat.get_visible_messages(t, me_id)
    reaction_data = service_chat.get_reaction_summaries([m.message_id for m in msgs], me_id)
    reply_lookup = {}
    for m in msgs:
...
def send_message_http(thread_id: int):
    ...
    if request.is_json:
        return jsonify({"ok": True, "message": service_chat.serialize_message(msg, reaction_summary)})
    return redirect(url_for("chat.thread", thread_id=thread_id))
...
def react_message(thread_id: int, message_id: int):
    ...
    if not emoji:
        if existing:
            ...
            db.session.commit()
        if request.is_json:
            summary = service_chat.get_reaction_summaries([message_id], me_id).get(message_id, {"counts": {}, "user_reaction": None})
...
def subscribe_broadcast(thread_id: int):
    # Procedure: subscribe_broadcast.
    me_id = get_current_user_id()
    thread = ChatThread.query.get_or_404(thread_id)
    if thread.thread_type != "broadcast":
        abort(400, "Not a broadcast")
    if service_chat.is_member(thread_id, me_id):
...
def unsubscribe_broadcast(thread_id: int):
    # Procedure: unsubscribe_broadcast.
    me_id = get_current_user_id()
    thread = ChatThread.query.get_or_404(thread_id)
    if thread.thread_type != "broadcast":
        abort(400, "Not a broadcast")
...
def api_messages(thread_id: int):
    ...
    msgs = service_chat.get_visible_messages(thread, me_id)
    reaction_map = service_chat.get_reaction_summaries([m.message_id for m in msgs], me_id)
    return jsonify([service_chat.serialize_message(m, reaction_map.get(m.message_id, {})) for m in msgs])
...
def api_poll(thread_id: int):
    ...
    while True:
        new_msgs = (
            service_chat.get_visible_messages(thread, me_id, since_id=since, limit=POLL_MAX_RETURN)
        )
        if new_msgs:
            ...
            if mem:
                ...
                db.session.add(mem)
                db.session.commit()
            reaction_map = service_chat.get_reaction_summaries([m.message_id for m in new_msgs], me_id)
            return jsonify({"messages": [service_chat.serialize_message(m, reaction_map.get(m.message_id, {})) for m in new_msgs]})
